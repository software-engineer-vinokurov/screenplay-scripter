...
class ReviewMenuBarApp(rumps.App):
    ...
    def __init__(self, script_name: str, pause_event, stop_event):
        super().__init__(f'{self._PLAY} {script_name}', quit_button=None)
...
def run_with_review_menubar(script_path, play_fn) -> None:
    ...
    app = ReviewMenuBarApp(script_path.name, pause_event, stop_event)
    ...
    def _player():
        play_fn(on_progress=app.update_progress, pause_event=pause_event, stop_event=stop_event)
        app.player_done.set()
